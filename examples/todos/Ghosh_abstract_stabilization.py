...
def ghosh():
    ...
    def axiom(sym):
        return And(axiom_leq_node(sym), abstraction_axioms(sym))
    ...
    def tr_wakeup(sym1, sym2, param):
        ...
        return And(
            ...
            Or(If(priv(sym1, n), move(sym1, sym2, n), move_idle(sym1, sym2, n))),
    ...
    counts_for_below_move = lambda sym, param: Or(
        And(
            ...
            ForAll(
                ...
                Implies(
                    And(
                        sym["leq_node"](param["x"], Y),
                        sym["leq_node"](Y, param["i"]),
                        param["i"] != Y,
                    ),
        ...
        And(
            ...
            ForAll(
                Y,
                Implies(
                    And(sym["leq_node"](Y, param["i"]), param["i"] != Y),
    ...
    counts_for_above_move = lambda sym, param: Or(
        And(
            ...
            ForAll(
                ...
                Implies(
                    And(
                        sym["leq_node"](Y, param["x"]),
                        sym["leq_node"](param["i"], Y),
                        param["i"] != Y,
                    ),
                    Not(priv_by_type(sym, Y, below)),
                ),
        ...
        And(
            ...
            ForAll(
                Y,
                Implies(
                    And(sym["leq_node"](param["i"], Y), param["i"] != Y),
    ...
    dist_below = ParPointwiseFreeRank(
        binary_counts_for_below, param_i, param_xt, hints_for_dist
    )
    dist_above = ParPointwiseFreeRank(
        binary_counts_for_above, param_i, param_xt, hints_for_dist
    )
    ...
    hints_B = [
        x_to_skd | t_to_above,
        x_to_skd | t_to_below,
        x_to_next | t_to_above,
        x_to_next | t_to_below,
        x_to_prev | t_to_above,
        x_to_prev | t_to_below,
    ]

    const_rank = BinaryFreeRank(lambda sym, param: True, param_xt)
    not_pred_token_xt = lambda sym, param: Not(pred_token_xt(sym, param))
    token_xt_and_distance_xt = LinFreeRank(
        [const_rank, distance_xt], [not_pred_token_xt, pred_token_xt], param_xt
    )
    sum_distances = ParPermute2FreeRank(
        token_xt_and_distance_xt, param_xt, {}, hints_A, hints_A, hints_B
    )
    rank = LexFreeRank([num_breaks, sum_distances])

    not_priv = lambda sym: Not(priv(sym, sym["skd"]))
    both_priv = lambda sym: And(
        priv_above(sym, sym["skd"]), priv_below(sym, sym["skd"])
    )
