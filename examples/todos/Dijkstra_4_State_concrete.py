...
def dijkstra4state():
    ...
    constant_sym = {"skd": Node, "top": Node, "bot": Node, "node_h": Node}
    relation_sym = {"leq_node": [Node, Node], "x": [Node], "up": [Node]}
    function_sym = {
        "next": [Node, Node],
        "prev": [Node, Node],
    }
    ...
    def induction_axiom(sym):
        return Implies(
            ...
            Exists(
                ...
                And(
                    sym["up"](Z) == False,
                    ForAll(Y, Implies(sym["up"](Y) == False, sym["leq_node"](Z, Y))),
                ),
            ),
        )

    def axiom(sym):
        return And(axiom_leq_node(sym), induction_axiom(sym))
    ...
    def priv_above(sym, i):
        return And(
            sym["x"](i) == sym["x"](sym["next"](i)),
            sym["up"](i),
            Not(sym["up"](sym["next"](i))),
            i != sym["top"],
        )
