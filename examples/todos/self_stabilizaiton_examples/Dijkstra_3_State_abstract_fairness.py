...
def dijsktra3():
    ...
    def stability_abstraction_axioms(sym):
        return And(
            ...
            Implies(
                ...
                ForAll(
                    X, Implies(X != sym["top"], sym["a"](X) == sym["a"](sym["bot"]))
                ),
            ),
        )
    ...
    def generalized_priv_below(sym, i):
        return Or(priv_below(sym, i), And(i == sym["top"], priv_top(sym, i)))
    ...
    after_h_below = lambda sym, param: And(
        generalized_priv_below(sym, param["i"]),
        sym["leq_node"](sym["node_h"], param["i"]),
    )
    before_h_below = lambda sym, param: And(
        generalized_priv_below(sym, param["i"]),
        sym["leq_node"](param["i"], sym["node_h"]),
    )
    priv_above_pred = lambda sym, param: priv_above(sym, param["i"])
    generalized_priv_below_pred = lambda sym, param: generalized_priv_below(
        sym, param["i"]
    )
