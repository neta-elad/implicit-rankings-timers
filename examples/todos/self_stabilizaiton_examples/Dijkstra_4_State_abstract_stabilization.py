...
def dijkstra4state():
    ...
    def abstraction_axioms(sym):
        ...
        return And(Exists(X, any_priv(sym, X)), inst_correct_for_skd(sym))
    ...
    r = lambda sym, param: Implies(
        Exists(X, any_priv(sym, X)), any_priv(sym, sym["skd"])
    )
    p = true
    q = stable
    ...
    sum_distances = ParPermute2FreeRank(
        ...
        hints_A,
        hints_A,
        hints_B,  ###Here if you remove the hints it times out (you can comment out the line)
    )
    rank = LexFreeRank([num_tokens, sum_distances])
