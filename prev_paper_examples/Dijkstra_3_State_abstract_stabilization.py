...
def dijsktra3state():
    ...
    def plus1mod3(x):
        return If(x == zero, one, If(x == one, two, zero))

    def plus2mod3(x):
        return If(x == zero, two, If(x == one, zero, one))
    ...
    def update_instrumentation(sym1, sym2):
        ...
        return And(
            ...
            ForAll(
                X,
                Implies(
                    And(X != skd, X != prev_skd), sym2["a_next"](X) == sym1["a_next"](X)
                ),
            ),
            ...
            ForAll(
                X,
                Implies(
                    And(X != skd, X != next_skd), sym2["a_prev"](X) == sym1["a_prev"](X)
                ),
            ),
        )
    ...
    def abstraction_axioms(sym):
        ...
        return And(
            Exists(X, priv(sym, X)), inst_correct_around_skd(sym), case10_axiom(sym)
        )

    def axiom(sym):
        return And(axiom_leq_node(sym), axiom_3_machines(sym), abstraction_axioms(sym))
    ...
    def tr_wakeup(sym1, sym2, param):
        ...
        return And(
            ...
            If(priv(sym1, skd), move(sym1, sym2), move_idle(sym1, sym2)),
    ...
    def equal_top_bottom(sym, x):
        ...
        return And(
            x == sym["top"], sym["a"](sym["top"]) == sym["a"](sym["bot"])
        )  # should we replace a(bot) with a_next(top)
    ...
    hint0a = (
        [skd_great1, top_equal1, top_equal2, top_equal3],
        [skd_great1, top_equal1, top_equal2, top_equal3],
    )  # 0 perms
    hint1a = (
        [prev_skd_small1, prev_skd_small2, skd_small1, skd_small2],
        [top_equal1, top_equal2, top_equal3, skd_great1],
    )  # 4 perms
    hint2a = (
        [skd_small1, skd_small2, top_equal1, top_equal2],
        [skd_great1, prev_skd_great1, top_equal1, top_equal2],
    )  # 2 perms
    hint3a = (
        [skd_small1, skd_small2, prev_skd_great1, top_equal1],
        [prev_skd_small1, prev_skd_small2, skd_great1, top_equal1],
    )  # 3 perms
    hint4a = hint0a  # 0 (succeeds with no hint)
    hint5a = (
        [prev_skd_great1, top_equal1, top_equal2, top_equal3],
        [skd_great1, top_equal1, top_equal2, top_equal3],
    )  # 1
    hint6a = (
        [prev_skd_great1, skd_great1, top_equal1, top_equal2],
        [skd_small1, skd_small2, top_equal1, top_equal2],
    )  # 2
    hint7a = (
        [prev_skd_small1, top_equal1, top_equal2, top_equal3],
        [prev_skd_great1, top_equal1, top_equal2, top_equal3],
    )  # 1 (hint 6 works aswell)
    hint8a = (
        [skd_small1, skd_small2, top_equal1, top_equal2],
        [prev_skd_small1, prev_skd_small2, top_equal1, top_equal2],
    )  # 2
    hint9a = (
        [top_equal1, top_equal2, top_equal3, prev_skd_great1],
        [prev_skd_small1, prev_skd_small2, skd_great1, prev_skd_great1],
    )  # 3
    hint10a = (
        [prev_skd_great1, skd_small1, skd_small2, top_equal1],
        [prev_skd_small1, prev_skd_small2, skd_great1, skd_small2, top_equal1],
    )  # 3
    ...
    hint4a_r = (
        ...
        [skd_great1, top_equal1, top_equal2, top_equal3],
        prev_skd_great1,
    )  # 0 (on its own succeeds with no hint)
    hint7a_r = (
        [prev_skd_small1, top_equal1, top_equal2, top_equal3],
        [prev_skd_great1, top_equal1, top_equal2, top_equal3],
        prev_skd_small2,
    ...
    rank_a = ParPermFreeRank_variant(
        binary_rank_a,
        {"x": Node, "t": SixSort},
        {},
        4,
    ...
    def predicate_for_b(sym, param):
        ...
        return Or(
            ...
            And(
                ...
                Exists(
                    Y,
                    And(sym["leq_node"](Y, x), Y != x, sym["a"](Y) != sym["a_next"](Y)),
                ),
            ),
            And(
                x == sym["top"],
    ...
    hint10b_r = (
        ...
        [x_skd],
        x_skd,
    )  # interesting case, has to do with abstraction

    rank_b = ParPermFreeRank_variant(
        binary_rank_b,
        {"x": Node},
        {},
        1,
    ...
    def diff_from_next(sym, param):
        x = param["x"]
        return And(x != sym["top"], sym["a"](x) != sym["a_next"](x))

    binary_diff = BinaryFreeRank(diff_from_next, {"x": Node})
    num_diff = ParPermFreeRank_variant(
        binary_diff,
        {"x": Node},
        {},
        1,
        ...
        return And(
            ...
            Or(
                ...
                And(
                    ...
                    Or(
                        plus1mod3(sym["a"](x))
                        == sym["a_next"](x),  # every i is counted for this x
                        And(
                            sym["a"](x) == plus1mod3(sym["a_next"](x)),
                            sym["leq_node"](x, i),
                            x != i,
    ...
    hints = [
        ...
        i_next | t_left,
        i_prev | t_left,
    ]  # this is just all possible hints

    binary_d = BinaryFreeRank(predicate_for_d, param_xit)
    sum_it = ParPointwiseFreeRank(
        binary_d, {"i": Node, "t": TwoSort}, {"x": Node}, hints
    )
    # sum_it.print_conserved(ts)
    hint_missing = ([x_prev_skd], [x_skd], x_prev_skd)

    rank_d = ParPermFreeRank_variant(
        sum_it,
        {"x": Node},
        {},
        1,
        [hint0b, hint5b],
        [hint1b_r, hint6b_r, hint10b_r, hint_missing],
    )
