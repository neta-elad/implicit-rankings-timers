...
def backprop_fixed_order():
    ...
    order_axiom = lambda sym: And(
        ForAll(
            [X, Y], Implies(And(sym["var_order"](X, Y), sym["var_order"](Y, X)), X == Y)
        ),
        ForAll(
            [X, Y, Z],
            Implies(
                And(sym["var_order"](X, Y), sym["var_order"](Y, Z)),
                sym["var_order"](X, Z),
            ),
        ),
    ...
    def init(sym):
        return And(
            succ(
                sym, sym["dummy_root"], sym["curr_var"]
            ),  # curr_var is initialized to the second minimal value
    ...
    def curr_assignment_unchanged(sym1, sym2):
        return ForAll(
            [X, b], sym2["curr_assignment"](X, b) == sym1["curr_assignment"](X, b)
        )
    ...
    def update_curr_assignment(sym1, sym2, var, value):
        return And(
            ...
            ForAll(
                ...
                Implies(
                    X != var,
                    sym2["curr_assignment"](X, b) == sym1["curr_assignment"](X, b),
                ),
            ),
        )
    ...
    def remove_assignment_var(sym1, sym2, var):
        return ForAll(
            [X, b],
            sym2["curr_assignment"](X, b)
            == And(sym1["curr_assignment"](X, b), X != var),
        )

    def result_unchanged(sym1, sym2):
        return And(sym2["sat"] == sym1["sat"], sym2["unsat"] == sym1["unsat"])
            ...
            Or(
                ...
                And(
                    ...
                    Exists(
                        ...
                        ForAll(
                            [X, b],
                            Implies(appears(C, X, b), curr_assignment(X, Not(b))),
                        ),
                    ),
    ...
    def inv(sym):
        ...
        return And(
            ...
            ForAll(
                X,
                Implies(
                    And(sym["var_order"](curr_var, X), curr_var != X),
                    unassigned(sym, X),
                ),
            ),
            ForAll(
                X,
                Implies(
                    And(sym["var_order"](X, curr_var), X != curr_var, X != root),
    ...
    def forward(sym):
        return And(
            unassigned(sym, sym["curr_var"]), sym["curr_var"] != sym["dummy_root"]
        )
    ...
    strict_order = lambda sym1, param1, param2: And(
        sym1["var_order"](param1["v"], param2["v"]), param1["v"] != param2["v"]
    )
    rank1 = ParLexFreeRank(
        BinaryFreeRank(ghost_assignment, v_dict), v_dict, strict_order
    )
    ...
    num_not_true = ParPointwiseFreeRank(
        BinaryFreeRank(assigned_not_true, v_dict), v_dict
    )
