...
def ring_leader():
    ...
    axiom_btw = lambda sym: And(
        ForAll(
            [X, Y, Z, W],
            Implies(And(sym["btw"](W, X, Y), sym["btw"](W, Y, Z)), sym["btw"](W, X, Z)),
        ),
        ForAll([W, X, Y], Implies(sym["btw"](W, X, Y), Not(sym["btw"](W, Y, X)))),
        ForAll(
            [W, X, Y],
            Or(sym["btw"](W, X, Y), sym["btw"](W, Y, X), W == X, W == Y, X == Y),
        ),
    ...
    axiom_le = lambda sym: And(
        ...
        ForAll(
            [A, B, C], Implies(And(sym["le"](A, B), sym["le"](B, C)), sym["le"](A, C))
        ),
    ...
    axiom_size = lambda sym: Exists(
        [X, Y, Z], And(X != Y, Y != Z, Z != X, ForAll(W, Or(W == X, W == Y, W == Z)))
    )
    ...
    tr_wakeup = lambda sym1, sym2, param: And(
        ...
        sym2["max"] == sym1["max"],
        If(
            sym1["sent_own"](param["n"]),
            If(
                ForAll(A, Not(sym1["pending"](A, param["n"]))),
                        ...
                        If(
                            ...
                            And(
                                ...
                                ForAll(
                                    ...
                                    == And(
                                        sym1["pending"](A, X),
                                        Not(And(A == B, X == param["n"])),
                                    ),
                                ),
                            ),
                            If(
                                sym1["le"](sym1["id"](param["n"]), B),
                                # B is higher - pass forward
                                And(
                                    ForAll(X, sym2["leader"](X) == sym1["leader"](X)),
                                    ForAll(
                                        X, sym2["sent_own"](X) == sym1["sent_own"](X)
                                    ),
                                    ForAll(
                                        [X, A],
                                        sym2["pending"](A, X)
            ...
            And(
                ForAll(X, sym2["leader"](X) == sym1["leader"](X)),
                ForAll(
                    X, sym2["sent_own"](X) == Or(sym1["sent_own"](X), X == param["n"])
                ),
                ForAll(
                    [X, A],
                    sym2["pending"](A, X)
                    == Or(
                        sym1["pending"](A, X),
                        And(A == sym1["id"](param["n"]), X == param["succ"]),
                    ),
                ),
            ),
        ),
    ...
    safety = lambda sym: ForAll(
        [X, Y], Implies(And(sym["leader"](X), sym["leader"](Y)), X == Y)
    )
    leader_max = lambda sym: ForAll(
        [X, Y], Implies(sym["leader"](X), sym["le"](sym["id"](Y), sym["id"](X)))
    )
    self_pending_highest_id = lambda sym: ForAll(
        [X, Y],
        Implies(sym["pending"](sym["id"](X), X), sym["le"](sym["id"](Y), sym["id"](X))),
    )
    no_bypass = lambda sym: ForAll(
        [X, Y, Z],
        Implies(
    ...
    inv_safety = lambda sym: And(
        safety(sym), leader_max(sym), self_pending_highest_id(sym), no_bypass(sym)
    )
    ...
    rho = lambda sym: And(
        ...
        # inv_safety(sym),
        ForAll([A, X], Implies(sym["pending"](A, X), Exists(Y, sym["id"](Y) == A))),
        Or(
            q(sym),
        ...
        ForAll(
            X,
            Implies(
                And(sym["sent_own"](X), Not(sym["leader"](X))),
                Exists(
                    Y,
    ...
    pred_pending_n1n2 = lambda sym, param: sym["pending"](
...
def ring_leader_without_messages():
    ...
    def send_own(sym1, sym2, i, iplus1):
        return And(
            ...
            If(
                sym1["le"](sym1["msg"](iplus1), sym1["id"](i)),
                And(
                    # pass id forward
    ...
    def process_msg(sym1, sym2, i, iplus1):
        return And(
            ForAll(X, sym2["sent_own"](X) == sym1["sent_own"](X)),
            If(
                sym1["msg"](i) == sym1["id"](i),
                ForAll(X, sym2["leader"](X) == Or(sym1["leader"](X), X == i)),
                ForAll(X, sym2["leader"](X) == sym1["leader"](X)),
            ),
            If(
                And(
                    sym1["le"](sym1["msg"](iplus1), sym1["msg"](i)),
                    sym1["le"](sym1["id"](i), sym1["msg"](i)),
                ),
                And(
                    ...
                    ForAll(
                        X,
                        Implies(
                            And(X != iplus1, X != i), sym2["msg"](X) == sym1["msg"](X)
                        ),
                    ),
                ),
                And(
    ...
    self_pending_highest_id = lambda sym: ForAll(
        [X, Y],
        Implies(sym["msg"](X) == sym["id"](X), sym["le"](sym["id"](Y), sym["id"](X))),
    )
    no_bypass = lambda sym: ForAll(
        [X, Y, Z],
        Implies(
    ...
    inv_safety = lambda sym: And(
        no_bypass(sym), self_pending_highest_id(sym), leader_max(sym), safety(sym)
    )
    # ts.check_inductiveness(inv_safety)
    ...
    rho = lambda sym: And(
        ...
        ForAll(
            X,
            Or(Exists(Y, sym["msg"](X) == sym["id"](Y)), sym["msg"](X) == sym["zero"]),
        ),
        Or(
            q(sym),
        ...
        ForAll(
            X,
            Implies(
                sym["msg"](X) == sym["id"](sym["max"]), sym["sent_own"](sym["max"])
            ),
        ),
    ...
    psi = lambda sym, param: Or(
        And(param["n"] == sym["max"], Not(sym["sent_own"](sym["max"]))),
