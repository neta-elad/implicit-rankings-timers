...
def ackermann():
    ...
    def axiom(sym):
        return And(
            ForAll(
                [X, Y, Z],
                Implies(And(sym["le"](X, Y), sym["le"](Y, Z)), sym["le"](X, Z)),
            ),
    ...
    def trans_step1(sym1, sym2, param):
        ...
        return And(
            ...
            ForAll(
                [X, Y],
                stack_post(X, Y)
                == And(stack_pre(X, Y), Not(And(X == len_pop, Y == m_pop))),
            ),
            m_post == m_pop,
    ...
    def trans_step3(sym1, sym2, param):
        ...
        return And(
            ...
            ForAll(
                [X, Y],
                stack_post(X, Y) == Or(stack_pre(X, Y), And(X == len_pre, Y == m_push)),
            ),
    ...
    rho = lambda sym: And(
        ...
        ForAll(
            [X, Y, Z, W],
            Implies(
                And(sym["stack"](X, Y), sym["stack"](Z, W), sym["le"](X, Z)),
                sym["le"](W, Y),
            ),
        ),
        ...
        ForAll(
            [X, Y, Z],
            Implies(
                And(sym["stack"](X, Y), Not(sym["le"](Z, Y))), sym["le"](sym["m"], Z)
            ),
        ),
    )
    ...
    order_formula = lambda sym, param1, param2: And(
